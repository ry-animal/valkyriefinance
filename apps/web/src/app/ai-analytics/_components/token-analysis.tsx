...
type TokenAnalysisData = NonNullable<Awaited<ReturnType<typeof trpc.ai.getTokenAnalysis.useMutation>>['data']>;
...
export function TokenAnalysis() {
    ...
    const getTrendColor = (trend: string) => {
        switch (trend) {
            case 'bullish': return 'bg-green-500/20 text-green-700';
            case 'bearish': return 'bg-red-500/20 text-red-700';
            ...
            <CardHeader>
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <CardDescription>Enter comma-separated token symbols for AI-driven analysis.</CardDescription>
                    </div>
                </div>
            </CardHeader>
            <CardContent>
                ...
                {analysisResult && (
                    <div>
                        <div className="flex justify-between items-baseline mb-2">
                            <h4 className="font-semibold">Analysis Result: <span className="text-muted-foreground font-normal">{analysisResult.overallTrend}</span></h4>
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {analysisResult.tokenAnalysis.map((token) => (
                                    <TableRow key={token.token}>
                                        ...
                                        <TableCell className={`text-right ${token.change_24h >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                            {(token.change_24h * 100).toFixed(2)}%
                                        </TableCell>
                                        <TableCell>
                                            <Badge variant="outline" className={getTrendColor(token.trend)}>{token.trend}</Badge>
                                        </TableCell>
                                        <TableCell className={`font-bold ${getRecommendationColor(token.recommendation)}`}>
            ...
            <Table>
                <TableHeader>
                    <TableRow>
                        ...
                        <TableHead><Skeleton className="h-5 w-[60px]" /></TableHead>
                        <TableHead><Skeleton className="h-5 w-[90px]" /></TableHead>
