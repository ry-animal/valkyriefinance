...
async function performHealthChecks(vault, storage) {
  ...
  const sharePrice = totalSupply.gt(0) ? totalAssets.div(totalSupply) : ethers.utils.parseEther("1");
...
async function detectAnomalies(healthData, storage) {
  ...
  if (previousMetrics.totalAssets) {
    ...
    if (assetChange > 0.1) { // 10% change threshold
      anomalies.push({
        type: 'SUDDEN_ASSET_CHANGE',
        severity: assetChange > 0.5 ? MONITORING_CONFIG.ANOMALY_SEVERITY_LEVELS.CRITICAL : MONITORING_CONFIG.ANOMALY_SEVERITY_LEVELS.HIGH,
  ...
  if (previousMetrics.sharePrice) {
    ...
    if (priceChange > 0.05) { // 5% price change threshold
      anomalies.push({
        ...
        severity: priceChange > 0.2 ? MONITORING_CONFIG.ANOMALY_SEVERITY_LEVELS.HIGH : MONITORING_CONFIG.ANOMALY_SEVERITY_LEVELS.MEDIUM,
    ...
    if (webhook && webhook.url) {
      try {
        await fetch(webhook.url, {
          ...
          body: JSON.stringify({
            content: `🚨 **Vault Anomaly Detected**\n` +
                    `**Type:** ${anomaly.type}\n` +
