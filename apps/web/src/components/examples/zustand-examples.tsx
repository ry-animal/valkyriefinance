      ...
      <CardContent className="space-y-4">
        ...
        {user && (
          <div>
            ...
            {user.ensName && <p className="text-sm text-muted-foreground">ENS: {user.ensName}</p>}
          </div>
        )}

        <div className="flex gap-2">
          <Button onClick={mockWalletConnect} disabled={isAuthenticated}>
            Mock Connect Wallet
          </Button>
          <Button onClick={mockWalletDisconnect} variant="outline" disabled={!isAuthenticated}>
            Disconnect Wallet
          </Button>
        </div>
      </CardContent>
...
export function UIStoreExample() {
  const { activeModal, openModal, closeModal, addNotification, toggleDarkMode, isDarkMode } =
    useUIStore();
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div>
          ...
          <Badge variant={activeModal ? 'default' : 'secondary'}>{activeModal || 'None'}</Badge>
        </div>

        <div className="flex gap-2 flex-wrap">
          <Button onClick={() => openModal('create-portfolio')} size="sm">
            Open Modal
          </Button>
          <Button onClick={closeModal} variant="outline" size="sm" disabled={!activeModal}>
            Close Modal
          </Button>
          <Button
            onClick={() =>
        ...
        <div>
          <p className="text-sm text-muted-foreground">Notifications ({notifications.length}):</p>
...
export function PortfolioStoreExample() {
  const { portfolios, addPortfolio, selectPortfolio, selectedPortfolioId } = usePortfolioStore();
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        <div>
          <p className="text-sm text-muted-foreground">Selected Portfolio:</p>
          <Badge>{selectedPortfolio?.name || 'None'}</Badge>
        </div>
        ...
        <div>
          <p className="text-sm text-muted-foreground">Portfolios ({portfolios.length}):</p>
          <div className="flex gap-2 flex-wrap">
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex gap-2">
          <Button onClick={mockConnect} disabled={isConnected}>
            Mock Connect
          </Button>
          <Button onClick={mockDisconnect} variant="outline" disabled={!isConnected}>
            Disconnect
          </Button>
          <Button onClick={mockTransaction} disabled={!isConnected} size="sm">
            Mock Transaction
          </Button>
        </div>
