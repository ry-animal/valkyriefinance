...
export function BrutalGrid({ children, cols = 12, className }: BrutalGridProps) {
  ...
  return <div className={cn('grid', gridClass, className)}>{children}</div>;
}
...
export function BrutalSection({ children, className, fullWidth = false }: BrutalSectionProps) {
  return (
    <section className={cn('py-8 px-4', fullWidth ? 'w-full' : 'max-w-7xl mx-auto', className)}>
      {children}
    </section>
  );
}
...
export function BrutalHeadline({ children, size = 'lg', className }: BrutalHeadlineProps) {
  ...
  return (
    <h1 className={cn(sizeClass, 'uppercase tracking-tight leading-none', className)}>
      {children}
    </h1>
  );
}
...
export function BrutalText({
  ...
  return <p className={cn(variantClass, sizeClass, className)}>{children}</p>;
}
